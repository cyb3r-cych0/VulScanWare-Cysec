...
def run_scan(state, target, url_limit, depth_limit):
    ...
    for url in urls:
        ...
        for inj in engine.injector.inject(url):
            ...
            findings = engine.plugin_manager.run_detectors(inj)
    ...
    stored_urls = engine.crawler.crawl(target) # perform second crawl to detect stored payload rendering
    all_urls = list(set(urls + stored_urls)) # combine previously crawled + new pages
